	...
	mEdges(0)
{
	...
	mMesh = new Mesh;
	if (!mMesh) {
		ERR("new failed %d", sizeof(Mesh));
		return;
	}
	memset(mMesh, 0x0, sizeof(Mesh));
	...
	switch (type) {
		...
		case VertexBuffer::OPENGL_BUFFER:
			mMesh->vertexes = createGLVertexBuffer(&vertexes2, &textCoord2);
			mMesh->faces = new GLElementBuffer(faces2);
			mEdges       = new GLElementBuffer(edges2);
			break;
	}
}
...
VertexBuffer *SoftBody::createGLVertexBuffer(vec3Array_t *vertexes, vec2Array_t *texCoords)
{
	GLVertexBuffer *buf;

	if (!vertexes || vertexes->size() == 0) {
		ERR("Invalid vertex array. Unable to create GLVertexBuffer");
		return NULL;
	}
	buf = new GLVertexBuffer(vertexes->size());
	if (!buf) {
		ERR("new failed %d", sizeof(GLVertexBuffer));
		return NULL;
	}
